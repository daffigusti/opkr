...
class NaviControl():
  ...
  def case_1(self):  # acc
      btn_signal = CruiseButtons.RES_ACCEL
      self.btn_cnt += 1
  ...
  def case_2(self):  # dec
      btn_signal = CruiseButtons.DECEL_SET
      self.btn_cnt += 1
      if self.target_speed == self.VSetDis:
        self.btn_cnt = 0
        self.seq_command = 3      # case_3 번으로 이동.      
  ...
  def auto_speed_control(self, CS, navi_speed):
    ...
    print('V curv speed : %d' % v_curv_speed)
    ...
    print('V curv speedcontrol : %s' % self.curvSpeedControl)
